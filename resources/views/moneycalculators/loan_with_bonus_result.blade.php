...
<div class="result-section">
    ...
    <div style="background: #fff; padding: 25px; border-radius: 12px; border: 2px solid #ddd; margin: 30px 0;">
        ...
        <p style="background: #fff3cd; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            💡 ボーナス月は通常より多く返済します。ボーナス月の支払額は太字で表示されます。
        </p>
        
        @php
            $firstMonths = array_slice($schedule, 0, 12);
            $lastMonths = array_slice($schedule, -12);
            $middleCount = count($schedule) - 24;
        @endphp
        ...
        <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    @php
                        $isBonus = $payment['month'] % 6 == 0;
                    @endphp
                    <tr style="border-bottom: 1px solid #eee; {{ $isBonus ? 'background: #fff3cd;' : '' }}">
                        <td style="padding: 12px; text-align: center; border: 1px solid #ddd;">
                            {{ $payment['month'] }}回目 {{ $isBonus ? '🎁' : '' }}
                        </td>
                        <td style="padding: 12px; text-align: right; border: 1px solid #ddd; {{ $isBonus ? 'font-weight: bold; color: #d97706;' : 'font-weight: bold;' }}">
                            {{ number_format(ceil($payment['monthlyPayment'] * 10000)) }} 円
                        </td>
                        ...
                        <td colspan="5" style="padding: 20px; text-align: center; background: #f8f9fa; border: 1px solid #ddd;">
                            <details style="cursor: pointer;">
                                ...
                                <table style="width: 100%; margin-top: 15px;">
                                    @foreach (array_slice($schedule, 12, $middleCount) as $payment)
                                    @php
                                        $isBonus = $payment['month'] % 6 == 0;
                                    @endphp
                                    <tr style="border-bottom: 1px solid #eee; {{ $isBonus ? 'background: #fff3cd;' : '' }}">
                                        ...
                                        <td style="padding: 8px; text-align: right; {{ $isBonus ? 'font-weight: bold; color: #d97706;' : 'font-weight: bold;' }} width: 20%;">
                                            {{ number_format(ceil($payment['monthlyPayment'] * 10000)) }} 円
                                        </td>
                    ...
                    @php
                        $isBonus = $payment['month'] % 6 == 0;
                    @endphp
                    <tr style="border-bottom: 1px solid #eee; {{ $isBonus ? 'background: #fff3cd;' : '' }} {{ $loop->last ? 'background: #e6f7ff;' : '' }}">
                        <td style="padding: 12px; text-align: center; border: 1px solid #ddd;">
                            {{ $payment['month'] }}回目 {{ $isBonus ? '🎁' : '' }}
                        </td>
                        <td style="padding: 12px; text-align: right; border: 1px solid #ddd; {{ $isBonus ? 'font-weight: bold; color: #d97706;' : 'font-weight: bold;' }}">
